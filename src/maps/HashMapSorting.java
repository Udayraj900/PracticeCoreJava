...
public class HashMapSorting {
	...
	public static void main(String[] args) {
		...
		Map<String, String> dtreeMapLambda = new TreeMap<>((Comparator<String>) (s1, s2) -> s2.compareTo(s1));
		...
		Map<String, String> result = codenames.entrySet().stream()
				.sorted(Map.Entry.comparingByValue(Comparator.reverseOrder())).collect(Collectors.toMap(
						Map.Entry::getKey, Map.Entry::getValue, (oldValue, newValue) -> oldValue, LinkedHashMap::new));

		codenames.entrySet().stream().sorted(Entry.comparingByValue(Comparator.reverseOrder())).collect(
				Collectors.toMap(Entry::getKey, Entry::getValue, (value1, value2) -> value1, LinkedHashMap::new));
		printMap(result);
	...
	private static Map<String, String> sortByValue(Map<String, String> unsortedMap) {
		...
		// provide a custom Comparator- Try switch the o1 o2 position for a different
		// order
		/*
		 * Collections.sort(list, new Comparator<Entry<String, String>>() {
		 * 
		 * @Override public int compare(Entry<String, String> s1, Entry<String, String>
		 * s2) { return s1.getValue().compareTo(s2.getValue()); } });
		 */

		Collections.sort(list, (Comparator<Entry<String, String>>) (s1, s2) -> s1.getValue().compareTo(s2.getValue()));
